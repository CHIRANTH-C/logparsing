...
# 4.1 Login to the sender email
def send_email(email_address,password,report1):
    EMAIL_ADDRESS = email_address
    EMAIL_PASSWORD = password

    msg = EmailMessage()
    msg['Subject'] = 'Attaching the logs of results from log_parser.py file'
...
found_logs = parse_logs(logs_list,regexp=r'(.*) W (.*): (.*): (.*10091.*)')
generate_report(found_logs)
send_email('chiranthchandrashekar123@gmail.com','cjwjvuyxhjcxnmzq','report.txt')